...
test$std <- (test$tasmax - test$tasmin)/4
test$precision <- (1/(test$std^2))
...
CR_SNAP <- ggplot(data = CR_4.5, mapping = aes(x = date)) +
  ...
  xlab("Year") +
  ylab("Mean Forecast Temperature (C)")+
  ylim(-5, 20) +
  theme_classic() +
...
CR_ObsData <- CR_ObsData %>%
  dplyr::select(date, Airport, Air_MonthAvg, Air_MonthMax, Air_MonthMin, Region, BVS, CAB, CAN, EYS, RHM, SQR, TIN, TIS, WDD)
CR_ObsData
```
...
```{r}
CR_ObsData <- read.csv("DataFiles/CR_PondAirTemp_mean.csv", header=TRUE)
head(CR_ObsData)
...
YF_ObsData <- YF_ObsData %>%
  dplyr::select(date, Airport, Air_MonthAvg, Air_MonthMax, Air_MonthMin, Region,  MP1, MP3, PL3, PL2, UBP3, MP5, MP8, UBP1, UBP2, PL1, UBP4)
YF_ObsData
```
...
CR_df$Air_MonthMax<- CR_4.5$tasmax
CR_df$Air_MonthMin <- CR_4.5$tasmin
...
YF_df$Air_MonthMax <- YF_4.5$tasmax
YF_df$Air_MonthMin <- YF_4.5$tasmin
...
CR_ObsData <- CR_ObsData %>%
  select(date, Airport, Air_MonthAvg, Air_MonthMax, Air_MonthMin, Region, BVS, CAB, CAN, EYS, RHM, SQR, TIN, TIS, WDD)
CR_ObsData
```
...
YF_ObsData <- YF_ObsData %>%
  select(date, Airport, Air_MonthAvg, Air_MonthMax, Air_MonthMin, Region,  MP1, MP3, PL3, PL2, UBP3, MP5, MP8, UBP1, UBP2, PL1, UBP4)
YF_ObsData
```
...
CR_df$Air_MonthMax <- CR_8.5$tasmax
CR_df$Air_MonthMin <- CR_8.5$tasmin
CR_df$Region <- rep("CR", nrow(CR_df))
...
YF_df$Air_MonthMax <- YF_8.5$tasmax
YF_df$Air_MonthMin <- YF_8.5$tasmin
YF_df$Region <- rep("YF", nrow(YF_df))
...
CR_45Temps<- ggplot(data = CR_45Forecast, mapping = aes(x = date)) +
  geom_ribbon(mapping = aes(ymin = Air_MonthMin, ymax = Air_MonthMax), fill = "darkgrey") +
  geom_line(mapping = aes(y = Air_MonthAvg)) +
  geom_vline(xintercept = as.Date("2021-01-01"), color = "red", linetype = "dashed", size = 1.5) +
  xlab("Year") +
