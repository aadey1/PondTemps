...
#code from the linear regression models Created by Luca Adelfio, Rachel Hughes, Amaryllis Adey, and Erik Curtis

Adapted here to fit code from Alyssa
...
#This section compares the climates of Cordova and Yakutat
...
##Section 2
# Comparing logger data to the airport data
...
df5a <- df5 %>%
  group_by(Date, Region, site) %>%
  summarise(Tmean = round(mean(Temp, na.rm = TRUE), 2),
            Tmin = ifelse(all(is.na(Temp)), NA, round(min(Temp, na.rm = TRUE), 2)),
            Tmax = ifelse(all(is.na(Temp)), NA, round(max(Temp, na.rm = TRUE), 2)),
            SD= round(sd(Temp, na.rm=T),2),
            .groups = 'drop')

#Missing Data again likely a QA/QC issue? - Erik
...
#Create a plot fo the mean temp through time
```{r}
...
df5c <- df5b %>%
  group_by(site, wkyr) %>%
  summarise(wkavg = round(mean(Tmean, na.rm = TRUE), 2),
            wkmax = if(anyNA(Tmax)) NA else round(max(Tmax, na.rm = TRUE), 2),
            wkmin = if(anyNA(Tmin)) NA else round(min(Tmin, na.rm = TRUE), 2),
            .groups = 'drop')
...
Fit linear models for air vs. air temps at Yakutat sites/airport
```{r}
...
# Here the files with the air temperature data are: 
