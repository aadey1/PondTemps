...
ggplot(df5b, aes(x = Date, y = Tmean)) +
  geom_line(color = "black") +  # Set color for consistency
  ylab(expression(paste("Air Temperature (°C)"))) +
  xlab("Date") +
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
  facet_wrap(~ site, ncol = 3) +  # Adjust number of columns as needed
  ...
  theme(
    ...
    axis.text.x = element_text(angle = 45, hjust = 1),  # Adjust angle to 45 for better fit
    strip.text = element_text(size = 16)
  )

#export plot
ggsave("YK_air_temps.png", width=10, height=8, units="in",dpi=300)
```
...
ggplot(na.omit(df5d), aes(x = Amonavg, y = monavg)) +
  ...
  ylab("Site Air Temperature (°C)") +
  xlab("Airport Air Temperature (°C)") +
...
#export plot
ggsave("YK_air_lm_reg.png", width=10, height=8, units="in",dpi=300)
```
...
ggplot(df1b, aes(x = Date, y = Tmean)) +
  geom_line(color = "black") +  # Set color for consistency
  ylab(expression(paste("Water Temperature (°C)"))) +
  xlab("Date") +
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
  facet_wrap(~ site, ncol = 3) +  # Adjust number of columns as needed
...
#export plot
ggsave("YF_water_temps.png", width=10, height=8, units="in",dpi=300)
```
...
#export plot
ggsave("CR_water_temps.png", width=10, height=8, units="in",dpi=300)
```
...
# Part 7 - Plotting the air temperatuer versus the pond temperatures

```{r}
# Reshape the data from wide to long format
long_df <- CR_PondAirTemp_mean %>%
  pivot_longer(
    cols = starts_with("TIN") | starts_with("SQR") | starts_with("TIS") | starts_with("CAB") | starts_with("EYS") | starts_with("RHM") | starts_with("BVS") | starts_with("CAN") | starts_with("WDD"), 
    names_to = "Pond", 
    values_to = "Water_Temperature_C"
  ) %>%
  filter(!is.na(Water_Temperature_C))  # Remove rows with NA in water temperature

# Create the panel plot
plot <- ggplot(long_df, aes(x = Air_MonthAvg, y = Water_Temperature_C)) +
  geom_hline(yintercept = 0, linetype = 2) +
  geom_vline(xintercept = 0, linetype = 2) +
  geom_abline(slope = 1, intercept = 0, color = "red", lty = 3) +
  geom_point(color = "darkgray") +
  # Add linear regression line only for Air_MonthAvg > 0
  geom_smooth(data = long_df %>% filter(Air_MonthAvg > 0), 
              method = "lm", color = "black", se = FALSE) +
  ylab("Water Temperature (°C)") +
  xlab("Air Temperature (°C)") +
  ggtitle("Copper River Delta") +
  facet_wrap(~ Pond, ncol = 3) +  # Adjust number of columns as needed
  theme_classic() +
  theme(
    plot.title = element_text(hjust = 0.5, size = 24),
    axis.title = element_text(size = 22),
...
ggsave("CRD_Air_Water_Temp.png", plot = plot, width = 10, height = 8)

# Assuming YF_PondAirTemp_mean is your new dataset

# Reshape the data from wide to long format
long_df_yf <- YF_PondAirTemp_mean %>%
  pivot_longer(
    cols = starts_with("MP1") | starts_with("MP3") | starts_with("MP5") | starts_with("MP8") | 
           starts_with("PL1") | starts_with("PL2") | starts_with("PL3") | starts_with("UBP1") | 
           starts_with("UBP2") | starts_with("UBP3") | starts_with("UBP4"), 
    names_to = "Pond", 
    values_to = "Water_Temperature_C"
  ) %>%
  filter(!is.na(Water_Temperature_C))  # Remove rows with NA in water temperature
...
# Create the panel plot
plot_yf <- ggplot(long_df_yf, aes(x = Air_MonthAvg, y = Water_Temperature_C)) +
  geom_hline(yintercept = 0, linetype = 2) +
  geom_vline(xintercept = 0, linetype = 2) +
  geom_abline(slope = 1, intercept = 0, color = "red", lty = 3) +
  geom_point(color = "darkgray") +
  # Add linear regression line only for Air_MonthAvg > 0
  geom_smooth(data = long_df_yf %>% filter(Air_MonthAvg > 0), 
              method = "lm", color = "black", se = FALSE) +
  ylab("Water Temperature (°C)") +
  xlab("Air Temperature (°C)") +
...
# Save the plot
ggsave("YF_PondAirTemp_mean.png", plot = plot_yf, width = 10, height = 8)
```
