#CRD Monthly Temp Changes RCP 4.5
...
Mod_CRD45$Year <- year(Mod_CRD45$time)
...
monthly_means <- calculate_monthly_averages(Mod_CRD45, 2012, 2020)
monthly_avg_2030s <- calculate_monthly_averages(Mod_CRD45, 2030, 2039)
monthly_avg_2060s <- calculate_monthly_averages(Mod_CRD45, 2060, 2069)
monthly_avg_2090s <- calculate_monthly_averages(Mod_CRD45, 2090, 2099)
...
DeltaMonthly_CRD45 <- combined_monthly_avg %>% filter(Decade != "2020s")

# Group by Month and Decade, calculate mean and standard deviation of differences
summary_stats2 <- DeltaMonthly_CRD45 %>%
...
DeltaMonth_CRD45_plot <- ggarrange(
  MonthlyStats_2030Plot, 
  MonthlyStats_2060Plot, 
  MonthlyStats_2090Plot, 
  ncol = 3, 
...
DeltaMonth_CRD45_plot

# Save the combined plot
ggsave("Corr_DeltaMonthCRD45.png", plot = DeltaMonth_CRD45_plot, width = 12, height = 3)

# Save data as CSV files for each decade
write.csv(MonthlyStats_2030, file = "Corr_MonthlyStatsCRD45_2030.csv", row.names = FALSE)
write.csv(MonthlyStats_2060, file = "Corr_MonthlyStatsCRD45_2060.csv", row.names = FALSE)
write.csv(MonthlyStats_2090, file = "Corr_MonthlyStatsCRD45_2090.csv", row.names = FALSE)
