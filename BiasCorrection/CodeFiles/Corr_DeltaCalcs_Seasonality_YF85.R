#YF Monthly Temp Changes RCP 8.5
...
monthly_means <- calculate_monthly_averages(Mod_YF85, 2012, 2020)
monthly_avg_2030s <- calculate_monthly_averages(Mod_YF85, 2030, 2039)
monthly_avg_2060s <- calculate_monthly_averages(Mod_YF85, 2060, 2069)
monthly_avg_2090s <- calculate_monthly_averages(Mod_YF85, 2090, 2099)
...
DeltaMonthly_YF85 <- combined_monthly_avg %>% filter(Decade != "2020s")

# Group by Month and Decade, calculate mean and standard deviation of differences
summary_stats2 <- DeltaMonthly_YF85 %>%
...
DeltaMonth_YF85_plot <- ggarrange(
  MonthlyStats_2030Plot, 
  MonthlyStats_2060Plot, 
  MonthlyStats_2090Plot, 
  ncol = 3, 
...
write.csv(MonthlyStats_2030, file = "Corr_MonthlyStatsYF85_2030.csv", row.names = FALSE)
write.csv(MonthlyStats_2060, file = "Corr_MonthlyStatsYF85_2060.csv", row.names = FALSE)
write.csv(MonthlyStats_2090, file = "Corr_MonthlyStatsYF85_2090.csv", row.names = FALSE)
