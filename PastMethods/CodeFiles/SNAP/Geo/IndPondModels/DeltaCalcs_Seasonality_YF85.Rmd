...
# Reading in the data
```{r}
MP1Mod <- read.csv("MP1Mod_85.csv", header=TRUE)
MP1Mod$date <- as.Date(MP1Mod$date)

MP3Mod <- read.csv("MP3Mod_85.csv", header=TRUE)
MP3Mod$date <- as.Date(MP3Mod$date)

MP5Mod <- read.csv("MP5Mod_85.csv", header=TRUE)
MP5Mod$date <- as.Date(MP5Mod$date)

MP8Mod <- read.csv("MP8Mod_85.csv", header=TRUE)
MP8Mod$date <- as.Date(MP8Mod$date)

PL1Mod <- read.csv("PL1Mod_85.csv", header=TRUE)
PL1Mod$date <- as.Date(PL1Mod$date)

PL2Mod <- read.csv("PL2Mod_85.csv", header=TRUE)
PL2Mod$date <- as.Date(PL2Mod$date)

PL3Mod <- read.csv("PL3Mod_85.csv", header=TRUE)
PL3Mod$date <- as.Date(PL3Mod$date)

UBP1Mod <- read.csv("UBP1Mod_85.csv", header=TRUE)
UBP1Mod$date <- as.Date(UBP1Mod$date)

UBP2Mod <- read.csv("UBP2Mod_85.csv", header=TRUE)
UBP2Mod$date <- as.Date(UBP2Mod$date)

UBP3Mod <- read.csv("UBP3Mod_85.csv", header=TRUE)
UBP3Mod$date <- as.Date(UBP3Mod$date)

UBP4Mod <- read.csv("UBP4Mod_85.csv", header=TRUE)
...
Delta_YF85 <- avgX50_table  # Use your actual dataframe
...
for (year in years) {
  ...
  Delta_YF85[[diff_col_name]] <- Delta_YF85[[year]] - Delta_YF85$`2020`
}

# Display the structure of Delta_YF85 to verify
str(Delta_YF85)
```
...
YF85_selected <- Delta_YF85 %>%
  dplyr::mutate(
    Mean_2030_2039 = rowMeans(select(., starts_with("diff_203"))),  # Select columns starting with "diff_203"
    SD_2030_2039 = rowSds(as.matrix(select(., starts_with("diff_203")))),  # Compute row-wise SD for the selected columns
    Mean_2060_2069 = rowMeans(select(., starts_with("diff_206"))),  # Select columns starting with "diff_206"
    SD_2060_2069 = rowSds(as.matrix(select(., starts_with("diff_206")))),  # Compute row-wise SD for the selected columns
    Mean_2090_2099 = rowMeans(select(., starts_with("diff_209"))),  # Select columns starting with "diff_209"
    SD_2090_2099 = rowSds(as.matrix(select(., starts_with("diff_209"))))  # Compute row-wise SD for the selected columns
  )
# Print the structure of the updated dataframe
str(YF85_selected)
```
...
ggsave("Output/GeoSNAP/DeltaYF85.png", plot = Delta_YF85_plot, width = 8, height = 3)
```
...
DeltaMonthly_YF85 <- combined_monthly_avg %>%
  filter(Year != 2020)
DeltaMonthly_YF85
```
...
# Save the combined plot
ggsave("Output/GeoSNAP/DeltaMonthYF85.png", plot = DeltaMonth_YF85_plot, width = 12, height = 3)

```

Create .csv files of the data for these graphs
```{r}
write.csv(MonthlyStats_2030, file = "MonthlyStatsYF85_2030.csv")
write.csv(MonthlyStats_2060, file = "MonthlyStatsYF85_2060.csv")
write.csv(MonthlyStats_2090, file = "MonthlyStatsYF85_2090.csv")
```
